...
/************************ HELPER FUNCTIONS **************************/
/* FUNCTION trim_array                                               */
/* Removes empty items at the end of an array                       */
/* PARAM myArray - The array that will be modified                  */
function trim_array(myArray)
{
...
/* Adds a variable to the beginning of an array                     */
/* PARAM newItem - The item to be added at array[0]                 */
/* PARAM myArray - The array that will be modified                  */
/* PARAM frontOrBack - Add the item to the beginning or end?        */
function add_to_array(newItem,myArray,frontOrBack)
{
...
/* FUNCTION omniture_qa_check                                       */
/* Detects if the visitor is looking at the staging site.  If they  */
/* are, this function shifts the hierarchy and s.prop's around so   */
/* things are tracked in the QA bucket                              */
/* PARAM args      - An array containing the H1, H2, H3, & H4 tags  */
function omniture_qa_check(hArray) {
    var url_str=document.location.toString();
    var domain = url_str.split(".com");
    var sub = domain[0].split(".");
    var stag=sub[0];
    for (var x=1; x<=(sub.length)-2; x++) { stag=stag + "." + sub[x] }
    
    var brides_domain = url_str.split("/");

    if ((stag == 'http://origin.stag')||(brides_domain[2] == 'sfcpdebd05:25470'))
    {
        // Bump everything down one notch
        hArray=add_to_array('condenet_QA',hArray,'front');
    }
    return hArray;
}

/* FUNCTION report_omniture                                         */
/* Shows Omniture tracking in a pop-up window for easier debugging  */
/* Arguments should appear in the following order:                  */
/* s.hier1, s.pageName, s.prop6, s.prop7, s.prop8, s.prop9          */
if (typeof report_omniture_activated == 'undefined') {
	report_omniture_activated=false;
}
report_omniture_qs=window.location.search;
if ((report_omniture_qs.search('report_omniture=true')>-1) || (report_omniture_activated))
{
	timestamp = Number(new Date());
	if (location.href.indexOf('file:') > -1)
	{
		report_omniture_win = window.open('http://cs.condenastdigital.com/cs/api/report_omniture/','report_omniture'+timestamp,'location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,width=900,height=200,top=5,left=5');
	} else 
	{
		report_omniture_win = window.open('/cs/api/report_omniture/','report_omniture'+timestamp,'location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,width=900,height=200,top=5,left=5');
	}
	report_omniture_activated=true;
}
function report_omniture(){if (report_omniture_activated)try{report_omniture_win.show_omniture_variables(arguments);}catch(e){}}

/********************************************************************/
/* Creative Services Source Tracking                                */
/* Version                                                          */
/* Last Updated 2009-04-30                                          */
/********************************************************************/
/* Description:                                                     */
/* Look for the first empty s.prop, and if we find one, replace it  */
/* with information about the source from which the visitor came.   */
/* Examples:                                                        */
/*    arrived via link inside current program ==> 'internal'        */
/*     User arrived via Conde Nast properties ==> 'cnd'             */
/*             User arrived via search engine ==> 'external'        */
/* arrived via bookmark/typed in/saved in tab ==> 'no_special_info' */
/*   via "Special Tracking" _s in querystring ==> _s variable       */
/********************************************************************/
/* Requirements:                                                    */
/* In the HTML that calls this file, a variable called              */
/* "internal_url" needs to be set.                                  */
/* Ex.                                                              */
/* If the URL is http://www.condenet.com/promo/hyundai/car.html     */
/* then internal_url = "promo/hyundai"                              */
/* If the URL is http://www.beautymarkblog.com                      */
/* then internal_url = "beautymarkblog.com"                         */
/**************************************************************** ***/

function creative_services_source_tracking(internal_url, hArray)
{
	/* Check for "Special Tracking" first */
	var csst_source = '';  // This is the var we will eventually tack on.
	var csst_query = window.location.search.substring(1);
	var csst_vars = csst_query.split("&");
	for (var csst_i=0;csst_i<csst_vars.length;csst_i++)
	{
		var csst_pair = csst_vars[csst_i].split("=");
		if (csst_pair[0] == '_s') 
		{
			/* Found the _s variable */
			csst_source=csst_pair[1];
		}
	}
	if (csst_source == '')
	{	
		/* "Special Tracking" wasn't found */
		csst_source = document.referrer;
		if (csst_source == '')
		{
			/* Referrer wasn't found */
			csst_source = 'no_special_info';
		} else
		{
			csst_source = csst_source.toLowerCase();
			/* TODO: Better error handling here */
			if (internal_url.length==0)
			{
				/* Default: the current directory */
				internal_url = [((location.href).substr(0,(location.href).lastIndexOf('/'))).toLowerCase()];
			}
			
			/* Determine if it's internal, cnd, or external */
			var is_internal=false;
			var check_for_external=true;
			for (i=0; i<internal_url.length; i++)
			{
				if (((csst_source+'').indexOf(internal_url[i])) != -1) is_internal = true;
			}
			if (csst_source.indexOf('facebook.') != -1) 
			{
				csst_source = 'external-facebook';
				is_internal = false;
				check_for_external = false;
			}
			if (csst_source.indexOf('twitter.') != -1)
			{
				csst_source = 'external-twitter';
				is_internal = false;
				check_for_external = false;
			}
			if (is_internal)
			{
				/* It's internal! */
				csst_source = 'internal';
			} else if (check_for_external)
			{
				/* Strip the http:// and get the domain name only */
				if (csst_source.indexOf('http://') != -1)
				{
					csst_source = csst_source.substr(7);
				}
				csst_source = csst_source.substr(0,csst_source.indexOf('/'));
				if (csst_source.indexOf('condenet.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('condenastdigital.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('concierge.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('epicurious.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('style.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('wired.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('allure.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('brides.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('cookiemag.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('glamour.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('gourmet.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('jane.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('luckymag.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('thenewyorker.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('portfolio.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('self.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('teenvogue.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('vanityfair.') != -1) csst_source = 'cnd';
				if (csst_source.indexOf('ym.com') != -1) csst_source = 'cnd';
				
				/* If all else fails... */
				if (csst_source != 'cnd') csst_source = 'external';
			}
		}
	}

	return add_to_array(csst_source,hArray,'back');
...
/* FUNCTION cs_track                                                */
/* Arranges all the levels of hierarchy and prepares the data for   */
/* entry into Omniture.                                             */
/* PARAM hArray - An array containing the H1, H2, H3, & H4 tags     */
function cs_track(pageName, hArray)
{
	/* ASSIGN H1 - H4 */
	var H1=''; var H2=''; var H3=''; var H4='';
	var levels = hArray.length;
	if (levels < 1) return; // Nothing was passed in.  Nothing to track!
	if (levels>0) H1 = hArray[0];
	if (levels>1) H2 = hArray[1];
	if (levels>2) H3 = hArray[2];
	if (levels>3) H4 = hArray[3];
	if (levels>4)
	{
		for (i=4; i<levels; i++)
		{
	...
	s.server=H1; s.prop6=H1;
	if (H2 != '') { s.prop7 = H1+":"+H2; } else { s.prop7=s.prop6; s.prop8=s.prop6; s.prop9=s.prop6; }			
	if (H3 != '') { s.prop8 = H1+":"+H2+":"+H3; } else { s.prop8=s.prop7; s.prop9=s.prop7; }			
	if (H4 != '') { s.prop9 = H1+":"+H2+":"+H3+":"+H4; } else { s.prop9=s.prop8; }
	s.hier1=hArray.join(",");
	s.pageName=pageName;
}
...
/* Arranges all the levels of hierarchy, checks for special cases,  */
/* and prepares the data for entry into Omniture, which happens in  */
/* the page itself                                                  */
/* PARAM arguments - An array containing the H1, H2*, H3*, and H4*  */
/*     tags   *=optional                                            */
function track_pageview(internalUrl, pageName)
{
	var H1; var H2; var H3; var H4;
	var hArray = [];
	for(var i=2; i<arguments.length; i++)
	{
		hArray[i-2]=arguments[i];
	}
	hArray = trim_array(hArray);
	hArray = omniture_qa_check(hArray);
	hArray = creative_services_source_tracking(internalUrl, hArray);

	/* ASSIGN OMNITURE VARIABLES */
	cs_track(pageName, hArray);
	report_omniture(s.hier1,s.pageName,s.prop6,s.prop7,s.prop8,s.prop9);

	/* MOBILE DEVICE TRACKING - Creates a duplicate track */
	//track_mobile_device_event(pageName,hArray);
}

/* FUNCTION track_event                                             */
/* Arranges all the levels of hierarchy, checks for special cases,  */
/* and SENDS the data to Omniture                                   */
/* PARAM hArray - An array containing the H1, H2, H3, & H4 tags  */
/* EXAMPLE: track_event('chevy','sweeps','entry_form')              */
function track_event(pageName)
{
	var hArray = [];
	for(var i=1; i<arguments.length; i++){
		hArray[i-1]=arguments[i];
	}
	hArray = trim_array(hArray);
	hArray = omniture_qa_check(hArray);
//	Below is commented out because we usually don't care about this on "flash events"
//	hArray = creative_services_source_tracking(hArray);

	/* DEFAULT PAGENAME IS (URL + "#event") */
	if (pageName == '')
	{ 
		pageName=document.location.toString()+'#js_event';
	} else
	{
		pageName+='#js_event';
	}
	...
	/* PERFORM TRACK */
	var s_code=s.t();if(s_code)document.write(s_code) // track
	report_omniture(s.hier1,s.pageName,s.prop6,s.prop7,s.prop8,s.prop9);
...
function track_mobile_device_event(pageName, hArray)
{
	return false;
	var deviceName='we_do_not_care';
	...
	if (deviceName != 'we_do_not_care')
	{
		/* ADD DEVICE NAME TO THE H1 TRACKING (IE "IPAD") */
		hArray[0] = hArray[0]+"_"+deviceName;
...
if(!s.campaign)
	s.campaign=s.getQueryParam('cmpid,mbid',':');
